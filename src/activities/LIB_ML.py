"""
Módulo com funções para estudo de Machine Learning no contexto de Amplificadores de pontência.
-------------------------------------------
...
def f_find_coef_analytically(exp):
    ...
    Returns:
        COEFS -> Coeficientes w e b do modelo. (Dict)
    """
    w, b = sp.symbols('w b')
...
def f_calculate_y_estimate(list_x_real, w, b):
    """
    Função que calcula a saída estimada de acordo com os parametros calculados.
    Args:
...
def f_plot_xy(xs, ys, labels=None, x_label="X", y_label="Y", title=None):
    """
    Plota múltiplas curvas X vs Y em um único gráfico usando Matplotlib.
